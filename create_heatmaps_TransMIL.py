...
parser.add_argument('--config_file', type=str,
					default="/home/mvries/Documents/GitHub/CLAM/heatmaps_dtfd/configs/config_template_transmil.yaml")
args = parser.parse_args()

sys.path.append('/home/mvries/Documents/GitHub/TransMIL')

from infer import extract_feats_for_heatmap

def infer_single_slide_dtfd(data, reverse_label_dict, k=1):
	logits, Y_prob, Y_hat, A = extract_feats_for_heatmap(data)

	print(A.shape)
	A = A[0, :, 0, :2037].mean(axis=0, keepdim=True)
	print(A.shape)
	...
	print('Y_hat: {}, Y: {}, Y_prob: {}'.format(Y_hat.detach().cpu().numpy(), 1,
...
if __name__ == '__main__':
	...
	for i in range(len(process_stack)):
		...
		mDataList = (features.unsqueeze(0), 1)

		wsi_object.saveSegmentation(mask_file)
		Y_hats, Y_hats_str, Y_probs, A = infer_single_slide_dtfd(data=mDataList,
																 reverse_label_dict=reverse_label_dict,
															 k=exp_args.n_classes)
		...
		os.makedirs('heatmaps_dtfd/results/', exist_ok=True)
		if data_args.process_list is not None:
			process_stack.to_csv('heatmaps_dtfd/results/{}.csv'.format(data_args.process_list.replace('.csv', '')), index=False)
		else:
			process_stack.to_csv('heatmaps_dtfd/results/{}.csv'.format(exp_args.save_exp_code), index=False)
	...
	# 			pass
	# 		else:
	# 			heatmap = wsi_object.visWSI(vis_level=vis_level,
