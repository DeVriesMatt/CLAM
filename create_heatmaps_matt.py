...
from Main_DTFD_MIL import extract_feats_for_heatmap


def infer_single_slide(reverse_label_dict, k=1):

    logits, Y_prob, Y_hat, A = extract_feats_for_heatmap()
    preds_str = np.array([reverse_label_dict[Y_hat]])

    return Y_hat, preds_str, Y_prob.numpy(), A.numpy()
...
if __name__ == '__main__':
    ...
    for i in range(len(process_stack)):
        ...
        Y_hats,Y_hats_str, Y_probs, A = infer_single_slide(reverse_label_dict)
